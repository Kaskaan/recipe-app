package com.konradlesiak.bootstrap;

import com.konradlesiak.domain.*;
...
public class InitialData implements ApplicationListener<ContextRefreshedEvent> {
    ...
    public InitialData(CategoryRepository categoryRepository, RecipeRepository recipeRepository, UnitOfMeasureRepository unitOfMeasureRepository) {
    ...
    private List<Recipe> getRecipes() {
        ...
        Optional<UnitOfMeasure> eachUomOptional = unitOfMeasureRepository.findByUnitName("Each");

        if (eachUomOptional.isEmpty()) {
            throw new RuntimeException("Expected UOM Not Found in Database!");
        }

        Optional<UnitOfMeasure> tableSpoonUomOptional = unitOfMeasureRepository.findByUnitName("Tablespoon");

        if (tableSpoonUomOptional.isEmpty()) {
            throw new RuntimeException("Expected UOM Not Found in Database!");
        }

        Optional<UnitOfMeasure> teaSpoonUomOptional = unitOfMeasureRepository.findByUnitName("Teaspoon");

        if (teaSpoonUomOptional.isEmpty()) {
            throw new RuntimeException("Expected UOM Not Found in Database!");
        }

        Optional<UnitOfMeasure> dashUomOptional = unitOfMeasureRepository.findByUnitName("Dash");

        if (dashUomOptional.isEmpty()) {
            throw new RuntimeException("Expected UOM Not Found in Database!");
        }

        Optional<UnitOfMeasure> pintUomOptional = unitOfMeasureRepository.findByUnitName("Pint");

        if (pintUomOptional.isEmpty()) {
            throw new RuntimeException("Expected UOM Not Found in Database!");
        }

        Optional<UnitOfMeasure> cupsUomOptional = unitOfMeasureRepository.findByUnitName("Cup");
